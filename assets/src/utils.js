...
/**
 * Toggle the left sidebar (conversations)
 */
export function toggleLeftSidebar() {
    const { elements, layoutState } = window;
    
    layoutState.leftSidebarCollapsed = !layoutState.leftSidebarCollapsed;
    
    if (layoutState.leftSidebarCollapsed) {
        elements.appLayout.classList.add('left-collapsed');
    } else {
        elements.appLayout.classList.remove('left-collapsed');
    }
    
    // Handle both sidebars collapsed
    updateBothCollapsedState();
    
    // Save state
    localStorage.setItem('leftSidebarCollapsed', layoutState.leftSidebarCollapsed);
}
...
export function toggleRightSidebar() {
    ...
    if (layoutState.rightSidebarCollapsed) {
        elements.appLayout.classList.add('right-collapsed');
    } else {
        elements.appLayout.classList.remove('right-collapsed');
    }
    ...
    // Save state
    localStorage.setItem('rightSidebarCollapsed', layoutState.rightSidebarCollapsed);
}

/**
 * Update the both-collapsed state
 */
function updateBothCollapsedState() {
    const { elements, layoutState } = window;
    
    if (layoutState.leftSidebarCollapsed && layoutState.rightSidebarCollapsed) {
        elements.appLayout.classList.add('both-collapsed');
    } else {
        elements.appLayout.classList.remove('both-collapsed');
    }
}